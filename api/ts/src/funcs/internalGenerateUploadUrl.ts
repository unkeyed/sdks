 ...
 */
export function internalGenerateUploadUrl(
  client: UnkeyCore,
  request: components.V2DeployGenerateUploadUrlRequestBody,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.V2DeployGenerateUploadUrlResponseBody,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.V2DeployGenerateUploadUrlRequestBody$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/v2/deploy.generateUploadUrl")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.V2DeployGenerateUploadUrlResponseBody$inboundSchema),
