...
// CreateDeploymentV3 - Create deployment
// Create a deployment for an app in a project.
//
...
//
// If set, this operation will use [Security.RootKey] from the global security.
func (s *Deployments) CreateDeploymentV3(ctx context.Context, request components.V3DeploymentsCreateDeploymentRequestBody, opts ...operations.Option) (*operations.DeploymentsCreateDeploymentV3Response, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionRetries,
		operations.SupportedOptionTimeout,
	...
	opURL, err := url.JoinPath(baseURL, "/v3/deployments.createDeployment")
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.DeploymentsCreateDeploymentV3Response{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 201:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.V3DeploymentsCreateDeploymentResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.V3DeploymentsCreateDeploymentResponseBody = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
