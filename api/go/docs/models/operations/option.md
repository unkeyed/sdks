...
```go
unkey.WithServerURL("https://api.example.com")
```
...
```go
unkey.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
unkey.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
unkey.WithTimeout(30 * time.Second)
```
